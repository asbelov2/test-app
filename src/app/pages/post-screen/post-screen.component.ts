...
export class PostScreenComponent {
  ...
  constructor(private http: HttpClient, private router: Router, private userService: UserService, private activatedRoute: ActivatedRoute) {
    activatedRoute.params.subscribe(next => {
      if ((next as any)?.id) {
        ...
        this.http.get(`http://localhost:5058/api/Post/${(next as any).id}`).subscribe(next => {
  ...
  createPost(event: Event) {
    ...
    this.http.get(`http://localhost:5058/api/user/CheckEmail?email=${this.userService.email}`).subscribe(next => {
      this.post.userId = (next as any).id
      this.http.post('http://localhost:5058/api/Post/', this.post, httpOptions).subscribe((next) => {
  ...
  updatePost(event: Event) {
    ...
    this.http.get(`http://localhost:5058/api/user/CheckEmail?email=${this.userService.email}`).subscribe(next => {
      this.post.userId = (next as any).id
      this.http.put(`http://localhost:5058/api/Post/${this.post.id}`, this.post, httpOptions).subscribe((next) => {
        this.router.navigateByUrl('/profile')
      })
    })
  }
